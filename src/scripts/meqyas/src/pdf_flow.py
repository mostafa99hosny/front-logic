...
# pdf_flow.py
# (inline-first + always open 3-dots; returns PDF metadata dict with file path)
#
# Behavior:
# 1) Inline PRIMARY action:
#    - Click ONLY if it clearly indicates Print/PDF (Arabic/English).
#    - If it looks like View/عرض (or anything else), skip.
#    - If clicked: press F10, download from new tab, close tab. source="inline"
# 2) ALWAYS open the 3-dots settings:
#    - If the FIRST <li> is PDF → click it → F10 → download → close tab → REOPEN menu
#      then click "عرض نموذج التقييم". source="dropdown"
#    - Else → click "عرض نموذج التقييم".
#
# Returns a dict:
#   {"ok": bool, "url": str|None, "filename": str|None, "path": str|None, "source": str|None}

# pdf_flow.py (inline + dropdown, returns bool)
# Behavior:
# 1) Inline primary action:
#    - If primary button clearly indicates Print/PDF (Arabic/English) → click → (optional dialog) → F10 → download.
#    - If it looks like View/عرض (or anything else) → skip.
# 2) Three-dots dropdown:
#    - Always open settings.
#    - If FIRST <li> is "طباعة (PDF)" → click & download, then reopen menu and click "عرض نموذج التقييم".
#    - Else → directly click "عرض نموذج التقييم".
#
# NOTE: Function returns True if a PDF was downloaded by any path; False otherwise.





# pdf_flow.py (inline + dropdown, returns (bool, pdf_meta))
# Behavior:
# 1) Inline primary action:
#    - If primary button clearly indicates Print/PDF (Arabic/English) → click → (optional dialog) → F10 → download.
#    - If it looks like View/عرض (or anything else) → skip.
...
#    - Else → directly click "عرض نموذج التقييم".
#
# Returns: (downloaded: bool, pdf_meta: {"pdf_file_path": Optional[str], "pdf_url": Optional[str]})
...
# Primary (inline) main action in the row
PRIMARY_ACTION_LOCATOR = (
    'button.style_viewBtn__vhvib.mainbutton, '
    'button.mainbutton:has(label)'
)

# 3-dots settings button & dropdown
SETTINGS_BUTTON      = 'button.dropdown-open.style_settingBtn__GFzkL, button.style_settingBtn__GFzkL'
DROPDOWN_CONTAINER   = 'div.rc-dropdown.rc-dropdown-placement-bottomLeft'
DROPDOWN_MENU        = 'ul.rc-dropdown-menu.style_actionsMenuContainer__Ntqdj'
MENU_FIRST_LI        = f'{DROPDOWN_MENU} > li.rc-dropdown-menu-item.style_actionsMenuItem__3ILTs, ' \
                       f'{DROPDOWN_MENU} > li.rc-dropdown-menu-item.style_groupSeperator__zjlrK.style_actionsMenuItem__3ILTs'
EVAL_MENU_BTN        = f'{DROPDOWN_MENU} button:has(label:has-text("عرض نموذج التقييم"))'

# Optional print-dialog stabilizers (best effort)
PRINT_DIALOG_CANDIDATES = [
    'div.fitHeightDialog.style_actionDailog__BXkDP.modalDialog',
    '[role="dialog"]:has-text("أختر التقرير")',
    '[role="dialog"]:has-text("اختر التقرير")',
...
# Accept URLs like .../print-valuation-pdf/<something>.pdf
PDF_URL_RE = re.compile(r"/print-valuation-pdf/[^/?#]+\.pdf", re.I)
...
async def _direct_download_from_url(page: Page, url: str, item_hint: Optional[str]) -> Tuple[bool, Optional[str], Optional[str]]:
    """
    Directly GET the PDF URL and save to DOWNLOAD_DIR.
    Returns (ok, file_path, url)
    """
    try:
        log(f"Attempting direct download: {url}")
        r = await page.context.request.get(url, timeout=600_000)
...
async def _open_settings_menu(page: Page) -> bool:
    """
    Click the 3-dots settings and wait for dropdown shell.
    """
    try:
        btn = page.locator(SETTINGS_BUTTON).first
        await btn.wait_for(state="visible", timeout=5_000)
        await btn.click()
        log("Clicked 3-dots settings button.")
        await page.locator(DROPDOWN_CONTAINER).first.wait_for(state="visible", timeout=5_000)
        await page.locator(DROPDOWN_MENU).first.wait_for(state="visible", timeout=5_000)
        log("Settings dropdown is visible.")
        return True
    except Exception as e:
        log(f"Failed to open settings menu: {e}", "WARN")
...
async def _menu_click_eval(page: Page) -> bool:
    """
    Click 'عرض نموذج التقييم' in the open dropdown.
    Returns True if clicked, False otherwise.
    """
    try:
        btn = page.locator(EVAL_MENU_BTN).first
        if await btn.count() == 0 or not await btn.is_visible():
            log('Dropdown: "عرض نموذج التقييم" not present/visible.', "WARN")
            return False
        await btn.click()
        log('Dropdown: clicked "عرض نموذج التقييم".')
        return True
    except Exception as e:
        log(f'Failed to click "عرض نموذج التقييم": {e}', "WARN")
        return False

async def _menu_click_first_if_pdf_else_eval(page: Page, item_hint: Optional[str]) -> Tuple[bool, Dict[str, Optional[str]]]:
    """
    In an already-open menu:
      - If FIRST li looks like "طباعة (PDF)" → click & download; then reopen + click "عرض نموذج التقييم".
      - Else → directly click "عرض نموذج التقييم".
    Returns (downloaded, pdf_meta)
    """
    downloaded = False
    pdf_meta: Dict[str, Optional[str]] = {"pdf_file_path": None, "pdf_url": None}

    try:
        first_li = page.locator(MENU_FIRST_LI).first
        await first_li.wait_for(state="visible", timeout=5_000)

        title = (await first_li.get_attribute("title")) or ""
        title_norm = _norm_text(title)
        label_text = ""
        try:
            label_el = first_li.locator("label").first
            if await label_el.count() > 0:
                label_text = _norm_text(await label_el.text_content())
        except Exception:
            pass

        is_pdf_first = ("pdf" in title_norm) or ("طباعة" in title_norm) or \
                       ("pdf" in label_text) or ("طباعة" in label_text) or ("print" in label_text)

        if is_pdf_first:
            # Click first li (PDF)
            try:
                await first_li.locator("button").first.click()
                log('Dropdown: clicked FIRST item "طباعة (PDF)".')

                # Run F10 → wait for new tab → download
                await _maybe_wait_dialog_ready(page)
                await _fire_f10(page)
                new_tab = await _wait_for_new_pdf_tab(page)
                if new_tab:
                    ...
                    if not PDF_URL_RE.search(url):
                        try:
                            ...
                            pass
                    if url:
                        ok, file_path, url_final = await _direct_download_from_url(page, url, item_hint)
                        downloaded = downloaded or ok
                        pdf_meta["pdf_file_path"] = file_path
                        pdf_meta["pdf_url"] = url_final
                    else:
                        log("New tab did not expose a URL.", "WARN")
                    try:
                        await new_tab.close()
                ...
                log(f"Failed to click first (PDF) menu item: {e}", "WARN")

            # Re-open the menu to click "عرض نموذج التقييم"
            if await _open_settings_menu(page):
                await _menu_click_eval(page)
            else:
                log("Could not reopen settings menu to click eval.", "WARN")
        else:
            await _menu_click_eval(page)
    ...
    return downloaded, pdf_meta

# --- Main API -----------------------------------------------------------------

async def try_download_pdf(page: Page, item_hint: Optional[str] = None) -> Tuple[bool, Dict[str, Optional[str]]]:
    """
    Flow:
      1) Inline: If primary button is PDF/Print-like → click → (optional dialog) → F10 → download.
         (If inline looked like View/عرض, skip.)
      2) Open 3-dots:
         - If FIRST li is PDF → click & download → reopen menu → click "عرض نموذج التقييم".
         - Else → click "عرض نموذج التقييم".
    Returns (downloaded, pdf_meta) where pdf_meta has {"pdf_file_path", "pdf_url"}.
    """
    downloaded = False
    pdf_meta: Dict[str, Optional[str]] = {"pdf_file_path": None, "pdf_url": None}
    ...
    try:
        ...
        if await primary.count() > 0 and await primary.is_visible():
            ...
            log("Primary action button not present/visible; skipping inline.", "WARN")
    except Exception as e:
        log(f"Primary inline detection failed: {e}", "WARN")

    # 2) Menu path (always open settings now)
    if await _open_settings_menu(page):
        menu_downloaded, menu_pdf_meta = await _menu_click_first_if_pdf_else_eval(page, item_hint)
        downloaded = downloaded or menu_downloaded
        # prefer inline path unless menu provided something new
        for k, v in menu_pdf_meta.items():
            if not pdf_meta.get(k) and v:
                pdf_meta[k] = v
    else:
        log("Could not open settings menu for dropdown handling.", "WARN")
